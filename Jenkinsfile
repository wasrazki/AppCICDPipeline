pipeline{
    ...
    stages{
        ...
        stage("Trigger the CD Pipeline "){
            steps{
                script{
                    sh "curl -v -k --user ${Jenkins_Username}:${Jenkins_API_access_token} -X POST -H 'cache-control:no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://${Jenkins_Controller}/job/gitops-AppCICIPipeline/buildWithParameters?token=gitops-pipeline-token' " 
